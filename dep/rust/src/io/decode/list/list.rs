...
impl Decoder {
        ...
        match wire {
            WireType::List => {
                ...
                self.decode_list_value(header, r, |r, first| {
                    decode_fn(header.element_wire_type(), r, first)
                })
            }
        ...
        F: Fn(&mut Take<&mut R>, u8) -> Result<T, DecodingError>,
    {
        ...
        let mut r: Take<&mut R> = Read::take(r, header.size_bytes() as u64);
