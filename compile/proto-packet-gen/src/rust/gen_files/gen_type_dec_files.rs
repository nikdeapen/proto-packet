...
use clerr::Report;
use code_gen::Source;
use proto_packet_tree::{ModPath, ModPathRef, TypeDec, WithTypeName};
        ...
        W: Writer,
    {
        ...
        for schema in schemas {
            for type_dec in schema.read()?.type_decs() {
                let mod_path: ModPathRef = schema.mod_path();
                let source: Source = match type_dec {
                    TypeDec::StructDec(s) => self.gen_struct(mod_path, &s),
                    TypeDec::MessageDec(m) => self.gen_message(mod_path, &m),
                    TypeDec::EnumDec(e) => self.gen_enum(e),
                    TypeDec::VariantDec(v) => self.gen_variant(mod_path, &v),
                };
